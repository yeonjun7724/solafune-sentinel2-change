...
- `tests/` — 65 pytest tests (unit + a real-data integration test), all passing.
...
4. Check the **Dependencies** tab — most Windows/OSGeo4W QGIS installs are
   missing `rasterio`/`scikit-learn`/`libpysal`/etc. (verified: this was
   actually the case on the QGIS 3.44.12 install used to test this plugin —
   see `docs/QGIS_PLUGIN_TEST_CHECKLIST.md`). Two ways to fix it, both
   field-verified end to end: install straight into QGIS's own Python with
   `python-qgis-ltr.bat -m pip install --user rasterio geopandas shapely
   pyproj scipy scikit-image PyYAML libpysal esda scikit-learn matplotlib
   folium` (no separate folder, Embedded mode just works), or point
   "External interpreter" at this project's `.venv` for full isolation. See
   `docs/QGIS_PLUGIN_USER_GUIDE.md` section 2 for the full comparison.
...
65 tests, all passing: band discovery (incl. missing/duplicate bands), CRS/
